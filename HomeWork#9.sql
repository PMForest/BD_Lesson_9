-- ДЗ БД Урок 8-9 Практическое задание по теме “Транзакции, переменные, представления” 
...
CREATE TRIGGER name_insert BEFORE INSERT ON products FOR EACH ROW
BEGIN
	IF NEW.name IS NULL AND.description IS NULL
		THEN SIGNAL sqlstate '45001' set message_text = "products name not be NULL";
	END IF;
END; //

CREATE TRIGGER name_update BEFORE UPDATE ON products FOR EACH ROW
BEGIN
